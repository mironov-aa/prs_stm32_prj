...
void SPI1_OnlyReceiveBlockDMA(uint8_t* bufferOut)
{
    ...
    DMA1_Channel3->CMAR = (uint32_t)&dummySend;
    DMA1_Channel3->CNDTR = 512; //Data size
...
void SPI2_OnlyReceive(uint8_t* bufferOut, uint32_t countBytes)
{
    ...
    //memory increment, read from peripheral, 8bit
    DMA1_Channel4->CCR = DMA_CCR_MINC;
    DMA1_Channel4->CMAR = (uint32_t)bufferOut;
    DMA1_Channel4->CNDTR = countBytes; //Data size
    /* DMA1 Channel5 SPI2_TX config */
    DMA1_Channel5->CCR = DMA_CCR_DIR; //read from memory
    DMA1_Channel5->CMAR = (uint32_t)&dummySend;
    DMA1_Channel5->CNDTR = countBytes; //Data size
